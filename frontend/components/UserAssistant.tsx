...
import { modelsGens, AppAtoms } from "lib/store";
...
const WaitingMessage = ({ message }: any) => {
  /*
  ...
  useEffect(() => {
    setWaitingMap((waitingMap: any) => ({ ...waitingMap, [message.dtm]: 1 }));
    // アニメーションのループ処理
    let currentIndex = 0;
  ...
  useEffect(() => {
    if (isTimeout) {
      setWaitingMap((waitingMap: any) => ({ ...waitingMap, [message.dtm]: 0 }));
    }
  }, [isTimeout]);
...
const UserAssistant = ({ message }: any) => {
  ...
  const deleteMessage =
    ({ dtm, role }: any) =>
    () => {
      setMessagesOnDeleteMode((messages: any) => messages.filter((msg: any) => msg.dtm !== dtm || msg.role !== role));
    };
  return (
    <div
    ...
    >
      {!isMessageDeleteMode ? (
        <MaterialButton name="content_copy" className="absolute top-1 right-1 invisible" groupHoverVisible onClick={copyMessage(message.content)} />
      ) : (
